...
| Trigger overview | partial | missing-detail | 3 | `show`(Any→OK cells)/tags/nested-scope honored; still missing `show_suppressed` and Recent-vs-Problems recency. Note: an "empty" widget can be a permissions gap — diagnosed live: the QA widget scopes to a group the app's API account can't read (the app correctly honors permissions; grant the account read access to fix) |
