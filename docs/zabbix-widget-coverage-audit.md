...
| System information | partial | missing-detail | 1 | Renders Zabbix's Parameter/Value/Details table — server running (green/red), frontend version, host/template/item/trigger tallies with Zabbix's color-split details via permission-scoped `countOutput` queries, and required nvps when the internal item is visible — verified on-device (templates count matched the frontend exactly). `info_type=1` still lists HA nodes. Rows the account can't compute are omitted. Remaining: server binary version / update status / users online (not exposed via the API to non-super-admins) |
...
| Gauge | partial | missing-detail | 4 | unit/decimal formatting, `description` label-macros, Zabbix's arc-fill design (colors sampled from the live frontend), the full "Show" checkbox set (Description/Value/Needle/Value arc/Scale — mapping verified against the live edit form), the needle (geometry + #F2F2F2 default sampled from the live SVG; value reflows below the pivot as Zabbix does), `needle_color`/`value_color`/`desc_color`/`empty_color`, and the 180°/270° `angle` all honored; only per-element size knobs (`value_size`/`desc_size`/...) remain |
...
| Trigger overview | partial | missing-detail | 3 | Renders Zabbix's host × trigger matrix — rotated trigger-name column headers, full severity-colored cells (green for OK in "Show: Any"), blank where a host lacks the trigger — verified on-device; `show`/tags/nested-scope honored; still missing `show_suppressed` and Recent-vs-Problems recency. Note: an "empty" widget can be a permissions gap (resolved for this server: the app's account was granted read on the previously-denied group) |
| Item history | partial | missing-detail | 2 | Reads `columns.N.itemid`/`show_lines`/value maps/`time_period`, numeric readings unit-scaled, per-column display `name` honored, `show_timestamp` honored (default hides the time column), and rows render Zabbix's interleaved "Name / Value" list (all fetched lines, newest first) — verified against the live widget; still missing trend backfill + per-column thresholds |
...
- ~~**Graph (classic) — `graphid` read with an exact-name lookup, missing the live `graphid.0` field.**~~ **Done** — a live classic-graph widget stores its reference under the indexed name `graphid.0`; the resolver now reads it with `firstIndexedValue`, so the widget renders its graph instead of falling through to "not supported". **Verified on-device** against the QA dashboard.
