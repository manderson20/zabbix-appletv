...
| System information | partial | missing-detail | 2 | `info_type=1` shows HA nodes and `isRunning` is derived from them (`hanode.get`); standalone still uses the API-success proxy for running. Zabbix renders a full Parameter/Value/Details table (server address, version-update status, host/template/item/trigger counts, users online, required nvps) — needs count queries, still open |
